...
/** anuke please WHY A DO EVEN NEED TO THIS */
public class DrawNormalTurret extends DrawBlock{
    ...
    public DrawNormalTurret(String basePrefix){
        this.basePrefix = basePrefix;
    }

    public DrawNormalTurret(){
    }
    ...
    public void getRegionsToOutline(Block block, Seq<TextureRegion> out){
        ...
        for(var tparts : ammoParts.values()){
            for(var part : tparts){
                part.getOutlines(out);
            }
        }
    ...
    public void draw(Building build){
        ...
        Drawf.shadow(preview, build.x + tb.recoilOffset.x - turret.elevation, build.y + tb.recoilOffset.y - turret.elevation, 90f); //because why not
        ...
        if(parts.size > 0){
            if(outline.found()){
                ...
                Draw.rect(outline, build.x + tb.recoilOffset.x, build.y + tb.recoilOffset.y, 0f); // Z E RRR O
                Draw.z(turretLayer);
            }
            ...
            var params = DrawPart.params.set(build.warmup(), 1f - progress, 1f - progress, tb.heat, tb.curRecoil, tb.charge, tb.x + tb.recoilOffset.x, tb.y + tb.recoilOffset.y, 90f); // burn in the hell
        ...
        if(ammoParts.size > 0 && tb.getAmmoContent() != null){
            float progress = tb.progress();
            var params = DrawPart.params.set(build.warmup(), 1f - progress, 1f - progress, tb.heat, tb.curRecoil, tb.charge, tb.x + tb.recoilOffset.x, tb.y + tb.recoilOffset.y, 90f);

            var tparts = ammoParts.get(tb.getAmmoContent());
            if(tparts != null){
                for(var part : tparts){
                    params.setRecoil(part.recoilIndex >= 0 && tb.curRecoils != null ? tb.curRecoils[part.recoilIndex] : tb.curRecoil);
                    part.draw(params);
    ...
    public void drawTurret(Turret block, TurretBuild build){
        if(block.region.found()){
            Draw.rect(block.region, build.x + build.recoilOffset.x, build.y + build.recoilOffset.y, 90f);
        }

        if(liquid.found()){
            Liquid toDraw = liquidDraw == null ? build.liquids.current() : liquidDraw;
            Drawf.liquid(liquid, build.x + build.recoilOffset.x, build.y + build.recoilOffset.y, build.liquids.get(toDraw) / block.liquidCapacity, toDraw.color.write(Tmp.c1).a(1f), 90f);
        }

        if(top.found()){
            Draw.rect(top, build.x + build.recoilOffset.x, build.y + build.recoilOffset.y, 90f);
        }
    }
    ...
    public void drawHeat(Turret block, TurretBuild build){
        ...
        Drawf.additive(heat, block.heatColor.write(Tmp.c1).a(build.heat), build.x + build.recoilOffset.x, build.y + build.recoilOffset.y, 90f, heatLayer);
    }
